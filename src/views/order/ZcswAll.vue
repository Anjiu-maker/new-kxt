...
async function loadData() { loading.value = true; try { const res = await getOrderList('/orderInfo/zcgd_order_list_all', params); if (res.data?.code === 200) { tableData.value = res.data.data?.records ?? []; pageInfo.total = res.data.data?.total ?? 0 } } finally { loading.value = false } }
...
function forwordAddOrder(row) { const isQuick = row.channelHandleType === 0; workbenchNav?.openCustomTab('zcswAll_cl_' + row.orderId, '暂存事务处理(' + row.orderNo + ')', isQuick ? '/order/quickAddOrder' : '/order/editOrder', '暂存事务处理', { orderId: row.orderId, taskId: row.taskId }) }
