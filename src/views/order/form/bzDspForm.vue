...
async function handleSubmit() {
  try {
    ...
    loading.value = true
    const res = await http.post('/orderInfo/bz_dsp', { ...formModel.value, orderId: props.row?.orderId, taskId: props.row?.taskId })
...
<template>
  <el-dialog v-model="innerVisible" title="班长督办审批" width="800px" :close-on-click-modal="false" append-to-body @closed="formModel = { remarks:'' }">
