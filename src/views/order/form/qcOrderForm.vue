...
async function handleSubmit() {
  try {
    ...
    loading.value = true
    const res = await http.post('/orderInfo/qcOrder', { ...formModel.value, orderId: props.row?.orderId })
...
<template>
  <el-dialog v-model="innerVisible" title="QC工单" width="800px" :close-on-click-modal="false" append-to-body @closed="formModel = { remarks:'' }">
