...
const columns = [
  ...
  { prop: 'haveSoundName', label: '录音', width: '60', showOverflow: true }
]
...
async function loadData() { loading.value = true; try { const res = await getOrderList('/orderInfo/zcgd_order_list', params); if (res.data?.code === 200) { tableData.value = res.data.data?.records ?? []; pageInfo.total = res.data.data?.total ?? 0 } } finally { loading.value = false } }
...
function forwordAddOrder(row) { const isQuick = row.channelHandleType === 0; workbenchNav?.openCustomTab('zcsw_cl_' + row.orderId, '暂存事务处理(' + row.orderNo + ')', isQuick ? '/order/quickAddOrder' : '/order/editOrder', '暂存事务处理', { orderId: row.orderId, taskId: row.taskId }) }
...
function playAudio(row) { if (row.haveSoundName && row.haveSoundName !== '无') { const baseApi = window.common?.baseApi || window.__KXT_CONFIG__?.baseApi || ''; audioUrl.value = baseApi + row.haveSoundName; audioRow.value = row } }
