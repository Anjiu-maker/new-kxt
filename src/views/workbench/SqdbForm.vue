...
async function handleSubmit() {
  ...
  if (!valid) return
  try {
    await ElMessageBox.confirm('您确认申请督办吗?', '提示', { confirmButtonText: '确定', cancelButtonText: '取消', type: 'warning' })
    loading.value = true
    const res = await http.post('/orderInfo/applyForSupervision', { orderId: props.row.orderId, remarks: formModel.value.remarks })
...
<template>
  <el-dialog v-model="innerVisible" title="申请督办" width="800px" :close-on-click-modal="false" append-to-body @closed="formModel.remarks=''; orderData={}">
    ...
    <el-form ref="formRef" :model="formModel" label-width="100px" style="margin-top:16px" :rules="{ remarks: [{ required: true, message: '请填写督办原因', trigger: 'blur' }, { max: 1000, message: '不超过1000个字符', trigger: 'change' }] }">
      <el-form-item label="督办原因" prop="remarks">
        <el-input v-model="formModel.remarks" type="textarea" :autosize="{ minRows: 4, maxRows: 10 }" placeholder="请填写申请督办原因，最多输入1000个字符" maxlength="1000" />
      </el-form-item>
    </el-form>
    <template #footer>
      <el-button @click="innerVisible = false">关闭</el-button>
      <el-button type="primary" :loading="loading" @click="handleSubmit">申请督办</el-button>
    </template>
  </el-dialog>
</template>
