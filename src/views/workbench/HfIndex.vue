...
defineProps({ pageName: { type: String, default: 'HfIndex' } })
...
function totalNumber(code) {
  const map = {
    dhfsw: { id:'hf_dhf',title:'待回访',url:'/flow/order/dhfsw',fullpath:'首页/待回访' },
    yyhf: { id:'hf_yyhf',title:'预约回访',url:'/flow/order/yyhf',fullpath:'首页/预约回访' },
    hf_csdhfgd: { id:'hf_csdhf',title:'超时待回访',url:'/flow/order/hf_csdhfgd',fullpath:'首页/超时待回访' },
    hf_yhfgd: { id:'hf_yhf',title:'已回访',url:'/flow/order/hf_yhfgd',fullpath:'首页/已回访' },
    hf_yhfcs: { id:'hf_yhfcs',title:'已回访次数',url:'/flow/order/hf_yhfcs',fullpath:'首页/已回访次数' }
  }
...
async function loadCharts() {
  try {
    ...
    const pieInst = initChart('gdqxzb')
    if (pieInst && gdhfjg.data?.code===200) {
      const d = gdhfjg.data.data
      const colors = ['#3aa0ff','#a5c340','#f75863','#999']
      pieInst.setOption(makeDonutOption(d.series||[], d.legend||[], colors))
      chartInstances['gdqxzb'] = pieInst
    }

    const lineInst = initChart('fpzqlzs')
    if (lineInst && hfaslqr.data?.code===200 && Array.isArray(hfaslqr.data.data)) {
      ...
      lineInst.setOption(makeLineOption(xData, [
        { name:'个人', data:personal, color:'#3aa0ff' },
        { name:'整体', data:overall, color:'#a5c340' }
      ]))
      chartInstances['fpzqlzs'] = lineInst
    }
    ...
    if (hfaslpie.data?.code===200) renderG('asfpl','#a5c340')(hfaslpie.data.data)
    if (cxhflpie.data?.code===200) renderG('fpzql','#ff606c')(cxhflpie.data.data)
  } catch {}
}
...
<template>
  <section class="hf-index" v-loading="loading">
    <div class="top-row">
      <div class="progress-panel"><p class="jrhf-title">今日回访进度</p><el-progress :percentage="Number(jrhfjd)" :stroke-width="15" :text-inside="true" class="jdt-style" /></div>
    ...
    <div class="main-layout">
      ...
      <div class="main-center">
        <section class="panel"><header class="panel-header"><div class="panel-title"><el-icon><DataLine /></el-icon><strong>回访结果（本账号当日结果）</strong></div></header>
          <div id="gdqxzb" style="height:200px;width:100%"></div>
          <h4 class="chart-subtitle">最近7日回访按时率（本账号）</h4>
          <div id="fpzqlzs" style="height:280px"></div>
        </section>
      </div>
      ...
      <div class="main-right">
        <section class="panel"><header class="panel-header"><div class="panel-title"><el-icon><DataLine /></el-icon><strong>最近7日办理情况</strong></div></header>
          <div class="gauge-grid">
            <div id="asfpl" style="height:280px"></div>
            <div id="fpzql" style="height:280px"></div>
          </div>
        </section>
      </div>
    </div>
...
.hf-index { min-height: calc(100dvh - 300px); padding: 18px; }
