...
async function handleSubmit() {
  try {
    ...
    loading.value = true
    const res = await http.post('/orderInfo/sptDb', { ...formModel.value, orderId: props.row?.orderId, taskId: props.row?.taskId })
...
<template>
  <el-dialog v-model="innerVisible" title="省平台督办" width="800px" :close-on-click-modal="false" append-to-body @closed="formModel = { remarks:'' }">
