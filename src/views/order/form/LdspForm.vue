...
async function handleSubmit() {
  try {
    await ElMessageBox.confirm('确认提交批示吗?', '提示', { confirmButtonText: '确定', cancelButtonText: '取消', type: 'warning' })
    loading.value = true
    const res = await http.post('/orderInfo/deptReportToIndicate', { remarks: formModel.value.remarks, orderId: props.row?.orderId, taskId: props.row?.taskId })
...
<template>
  <el-dialog v-model="innerVisible" title="部门上报领导批示" width="800px" :close-on-click-modal="false" append-to-body @closed="formModel = { remarks:'' }">
