...
async function handleSubmit() {
  try {
    await ElMessageBox.confirm('确认审核发布吗?', '提示', { confirmButtonText: '确定', cancelButtonText: '取消', type: 'warning' })
    loading.value = true
    const res = await http.post('/orderInfo/channelRelease', { ...formModel.value, orderId: props.row?.orderId })
...
<template>
  <el-dialog v-model="innerVisible" title="审核发布" width="800px" :close-on-click-modal="false" append-to-body @closed="formModel = { remarks:'' }">
