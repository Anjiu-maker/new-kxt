...
async function loadData() { loading.value = true; try { const res = await getOrderList('orderInfoSpecial/zcgdList', params); if (res.data?.code === 200) { tableData.value = res.data.data?.records ?? []; pageInfo.total = res.data.data?.total ?? 0 } } finally { loading.value = false } }
...
function forwordAddOrder(row) { workbenchNav?.openCustomTab('zcswSpecial_cl_' + row.orderId, '待办诉求处理(' + row.orderNo + ')', '/order/specialAddOrder', '待办诉求处理', { orderId: row.orderId, taskId: row.taskId }) }
