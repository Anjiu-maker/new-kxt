...
async function handleSubmit() {
  try {
    await ElMessageBox.confirm('确认申请重办吗?', '提示', { confirmButtonText: '确定', cancelButtonText: '取消', type: 'warning' })
    loading.value = true
    const res = await http.post('/orderInfo/sqcf', { ...formModel.value, orderId: props.row?.orderId, taskId: props.row?.taskId })
...
<template>
  <el-dialog v-model="innerVisible" title="申请重办" width="800px" :close-on-click-modal="false" append-to-body @closed="formModel = { remarks:'' }">
    <Orderinfo ref="orderInfoRef" />
    <el-form label-width="90px"  style="margin-top:12px">
      <el-form-item label="申请原因"><el-input v-model="formModel.remarks" type="textarea" :autosize="{ minRows:3 }" placeholder="申请原因" maxlength="500" /></el-form-item>
    </el-form>
